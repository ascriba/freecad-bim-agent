*Diese Dokumentation gibt es auch auf: [English](README.md), [Deutsch](README.de.md).*
...
* **Space/Raum-Volumen:** Erstellung von IFC-konformen Räumen auf Basis von Festkörpern oder Begrenzungsflächen mit `create_space` und `add_space_boundary`.  
...
* **Bridge-Start-Abfrage:** Beim Laden der Bridge öffnet sich ein Qt-Dialog (oder Konsolen-Prompt), der explizit um Erlaubnis fragt. Ohne Bestätigung blockt `run_python` mit einer Fehlermeldung. Nachträgliches Aktivieren erfordert einen FreeCAD-Neustart.
...
| create\_space | base\_object (opt.), name | Erstellt einen Raum (Arch Space) auf Basis eines Festkörpers oder als leeres Volumen. |
| add\_space\_boundary | space\_name, object\_name, faces (opt.) | Fügt Begrenzungsflächen zu einem Raum hinzu. |
| remove\_space\_boundary | space\_name, object\_name, faces (opt.) | Entfernt Begrenzungsflächen von einem Raum. |
...
| execute\_python | script (Python-Code) | **Sicherheitskritisch.** Führt nativen Python-Code im FreeCAD-Kontext aus. Wird per Qt-Dialog beim Bridge-Start aktiviert/deaktiviert. |
